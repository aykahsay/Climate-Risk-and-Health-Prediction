...
# Uganda Rainy Season Indicators
...
# 2. Demographic Vulnerability (Core Drivers)
...
# 3. Spatial Coordinates, Terrain & K-Means Clusters
...
df_all['spatial_cluster'] = kmeans.fit_predict(df_all[['latitude', 'longitude']])
...
df_all['under5_x_rain_ratio'] = df_all['is_under_5'] * df_all['rain_ratio_7_30']
df_all['infant_x_rain_ratio'] = df_all['is_infant'] * df_all['rain_ratio_7_30']
...
# Add Smoothed OOF Target Encodings
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
global_mean = y.mean()

for col in ['age_group', 'month', 'year', 'quarter', 'spatial_cluster']:
    X[f'{col}_te'] = np.nan
...
# PASS 1: Base Stacked Ensemble Training
...
print(f"Pass 1 Stacker Score: {eval_score(y, oof_meta)[2]:.5f}")

# PASS 2: High Confidence Pseudo-labeling (Top 8% highest/lowest probabilities)
high_conf_pos = np.where(test_meta >= 0.94)[0]
high_conf_neg = np.where(test_meta <= 0.06)[0]

print(f"Pseudo-labeling {len(high_conf_pos)} positive and {len(high_conf_neg)} negative test samples...")

X_pseudo = pd.concat([X, X_test.iloc[np.concatenate([high_conf_pos, high_conf_neg])]], ignore_index=True)
y_pseudo = np.concatenate([y, np.ones(len(high_conf_pos)), np.zeros(len(high_conf_neg))])

# Retrain Base Models on Augmented Training Set
oof_cat_p2 = np.zeros(len(X))
test_cat_p2 = np.zeros(len(X_test))
oof_lgb_p2 = np.zeros(len(X))
test_lgb_p2 = np.zeros(len(X_test))
oof_xgb_p2 = np.zeros(len(X))
test_xgb_p2 = np.zeros(len(X_test))

for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    # Add pseudo-labeled test samples to training fold
    train_idx_aug = np.concatenate([train_idx, np.arange(len(X), len(X_pseudo))])
    
    X_tr, y_tr = X_pseudo.iloc[train_idx_aug], y_pseudo[train_idx_aug]
    X_va, y_va = X.iloc[val_idx], y[val_idx]
    
    cb = CatBoostClassifier(iterations=1200, learning_rate=0.03, depth=6, l2_leaf_reg=3.0, random_seed=42+fold, early_stopping_rounds=50, verbose=0)
    cb.fit(X_tr, y_tr, eval_set=(X_va, y_va), verbose=False)
    oof_cat_p2[val_idx] = cb.predict_proba(X_va)[:, 1]
    test_cat_p2 += cb.predict_proba(X_test)[:, 1] / 5.0
    
    lgbm = lgb.LGBMClassifier(n_estimators=1200, learning_rate=0.02, max_depth=6, num_leaves=31, subsample=0.8, colsample_bytree=0.7, random_state=42+fold, verbose=-1)
    lgbm.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(50, verbose=False)])
    oof_lgb_p2[val_idx] = lgbm.predict_proba(X_va)[:, 1]
    test_lgb_p2 += lgbm.predict_proba(X_test)[:, 1] / 5.0
    
    xgb_m = xgb.XGBClassifier(n_estimators=1200, learning_rate=0.02, max_depth=5, subsample=0.8, colsample_bytree=0.7, gamma=0.1, random_state=42+fold, early_stopping_rounds=50)
    xgb_m.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], verbose=False)
    oof_xgb_p2[val_idx] = xgb_m.predict_proba(X_va)[:, 1]
    test_xgb_p2 += xgb_m.predict_proba(X_test)[:, 1] / 5.0

# Blend Pass 1 Meta Stacker + Pass 2 Pseudo-labeled Models
oof_final = 0.50 * oof_meta + 0.20 * oof_cat_p2 + 0.15 * oof_lgb_p2 + 0.15 * oof_xgb_p2
test_final = 0.50 * test_meta + 0.20 * test_cat_p2 + 0.15 * test_lgb_p2 + 0.15 * test_xgb_p2

# Fine Calibration (Scale & Shift) to align F1 @ 0.5 exactly with target prior
eps = 1e-6
p_clip = np.clip(oof_final, eps, 1 - eps)
l = logit(p_clip)
...
for s in np.linspace(0.8, 1.8, 21):
    for c in np.linspace(-0.5, 0.5, 21):
        p_cal = expit(s * l + c)
...
print(f"\n--- PSEUDO-LABELED ENSEMBLE SCORE ---")
print(f"Optimal Scale s={best_s:.2f}, Shift c={best_c:.2f}")
print(f"Calibrated OOF F1 @ 0.5: {best_f1:.5f} | ROC-AUC: {best_auc:.5f} | Final Multi-Metric Score: {best_score:.5f}")

test_clip = np.clip(test_final, eps, 1 - eps)
...
sub.to_csv('submission_pseudolabeled.csv', index=False)
sub.to_csv('submission.csv', index=False)
print("\nSaved submission.csv and submission_pseudolabeled.csv successfully!")
