...
# Extract District / Region from location string for GroupKFold
df_all['district'] = df_all['location'].apply(lambda s: str(s).split(',')[-2].strip() if len(str(s).split(',')) > 1 else str(s).strip())
...
# Ground elevation risk threshold in Uganda
...
drop_cols = ['ID', 'deathdate', 'deathdate_dt', 'is_climate_sensitive', 'is_train', 'location', 'zone', 'gender', 'hot_days_30d', 'district']
...
groups = train_clean['district'].values
X_test = test_clean[features].copy()

# Group Target Encodings
...
# Step 1: Base Model Ensemble across multiple seeds
print("--- RUNNING MULTI-SEED BLENDING ENSEMBLE ---")

seeds = [42, 100, 2026, 777, 999]
oof_blend_all = np.zeros(len(X))
test_blend_all = np.zeros(len(X_test))
...
for seed in seeds:
    ...
    for fold, (train_idx, val_idx) in enumerate(skf_seed.split(X, y)):
        ...
        # CatBoost
        cb = CatBoostClassifier(iterations=1200, learning_rate=0.025, depth=6, random_seed=seed+fold, early_stopping_rounds=50, verbose=0)
        ...
        # LightGBM
        lgbm = lgb.LGBMClassifier(n_estimators=1200, learning_rate=0.02, max_depth=6, num_leaves=31, random_state=seed+fold, verbose=-1)
        ...
        # XGBoost
        xgb_m = xgb.XGBClassifier(n_estimators=1200, learning_rate=0.02, max_depth=5, subsample=0.8, colsample_bytree=0.8, random_state=seed+fold, early_stopping_rounds=50)
        ...
        # Random Forest
        rf = RandomForestClassifier(n_estimators=300, max_depth=12, random_state=seed+fold, n_jobs=-1)
        rf.fit(X_imp.iloc[train_idx], y_tr)
        oof_rf[val_idx] = rf.predict_proba(X_imp.iloc[val_idx])[:, 1]
        test_rf += rf.predict_proba(X_test_imp)[:, 1] / 5.0
        
    seed_oof = 0.35 * oof_cat + 0.35 * oof_lgb + 0.20 * oof_xgb + 0.10 * oof_rf
    seed_test = 0.35 * test_cat + 0.35 * test_lgb + 0.20 * test_xgb + 0.10 * test_rf
    
    oof_blend_all += seed_oof / len(seeds)
    test_blend_all += seed_test / len(seeds)

f1_final, auc_final, score_final = eval_score(y, oof_blend_all)
print(f"\n--- 5-SEED AVERAGED ENSEMBLE ---")
print(f"Multi-Seed OOF F1 @ 0.5: {f1_final:.5f} | ROC-AUC: {auc_final:.5f} | Final Multi-Metric Score: {score_final:.5f}")

# Generate Submission File
sub = pd.DataFrame({
    'ID': test_clean['ID'],
    'TargetF1': (test_blend_all >= 0.5).astype(int),
    'TargetRAUC': test_blend_all
})

sub.to_csv('submission_multiseed.csv', index=False)
sub.to_csv('submission.csv', index=False)
print("\nSaved submission_multiseed.csv and submission.csv successfully!")
