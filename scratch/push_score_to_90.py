...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    # 1. CatBoost (with tuned depth and l2_leaf_reg)
...
print(f"MLP Net  OOF Score: {eval_score(y, oof_mlp)[2]:.5f}")

# Level 2 Meta-Stacker Optimization
...
print(f"\n--- LEVEL-2 STACKING META-LEARNER SCORE ---")
print(f"Meta-Learner OOF F1: {f1_score(y, (oof_meta>=0.5).astype(int)):.5f} | AUC: {roc_auc_score(y, oof_meta):.5f} | Score: {eval_score(y, oof_meta)[2]:.5f}")

# Post-processing Logit Shift & Power Calibration
eps = 1e-6
p_clip = np.clip(oof_meta, eps, 1 - eps)
l = logit(p_clip)
...
print(f"\n--- OPTIMAL STACKED & CALIBRATED SCORE ---")
...
print("\nSaved submission_stacked.csv successfully!")
